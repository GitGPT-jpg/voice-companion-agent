...
Voice Companion Agent is an AI companion product prototype. Instead of a simple chatbot that sends one prompt and returns plain text, it is designed as a full **AI product engineering** system:
...
- **Older conversation history** is summarized rather than truncated, preserving continuity while controlling token cost
...
| `sing` | Song requests | Match song → Cloud RVC generation → playback |
...
### Prerequisites
- Python 3.11+
- API keys: Anthropic (required), ElevenLabs (optional), Replicate (for singing)
...
| `ELEVENLABS_VOICE_ID` | Optional | ElevenLabs target voice ID |
